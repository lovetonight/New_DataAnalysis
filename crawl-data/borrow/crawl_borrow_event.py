...
def crawl_borrow_event(wallet_addresses, chain="ethereum", detail=True):
    ...
    for tmp in borrow_objects:
        ...
        if token_adress is not None:
            if detail:
                if borrow_events[wallet_address][contract_address].get(token_adress) is not None:
                    borrow_events[wallet_address][contract_address][token_adress] += amount * token_price_dict[token_adress]
                else:
                    borrow_events[wallet_address][contract_address][token_adress] = amount * token_price_dict[token_adress]
            else:
                if borrow_events[wallet_address].get(contract_address) is not None:
                    total_borrow[wallet_address][contract_address] += amount * token_price_dict[token_adress]
                else:
                    total_borrow[wallet_address][contract_address] = amount * token_price_dict[token_adress]
    if detail:
